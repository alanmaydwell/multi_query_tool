...
import pyodbc,tkinter.filedialog,os,time
import pickle
...
v0.4
Added exception handling for to QueryData.execute() for pyodbc.ProgrammingError
and pyodbc.Error. Added self.errors to QueryData.execute() to hold any of these errors.
Modified QueryData.writeframe() to automatically display self.errors
These new error messages now included in HTML report generation.

Shifted runtime measurement to a slightly earlier step, so still measured
even when exception raised.

SQL load buttons now default to the previous file's location.
SQL query path field background changed to grey

v0.5
Added Align button. Column widths from topmost query are applied to other queries.
Only affects columns common to both queries.

Changed default "Include" tickbox setting to ticked.
A few cosmetic tweaks to widget positions.

v0.6 Add
new QueryPanel.reload_all_sql() method added. This means the SQL path
fields can be usefully updated manually.

v0.7
Made some changes to TestRunGui
(i)to make scrollbar handling more expandable
, so possibly extra scrollbars could be added later.
(ii) Added option to display results in separate window.

[Improvements to make - Change option of export format in plain text?]


v0.8
Updated QueryData.writeframe() to have limit on number of rows to display.
to optional max_rows argument with 1000 (default values).
There seems to be a fundamental Tkinter grid limit of 10000 rows per Frame.
In addition GUI can crash with even fewer than 10000. 1000 seems to be OK.
...
class QueryPanel:
    ...
    def __init__(self,frame,title="SQL Query Definitions",defaultfile='default.dat'):
        ...
        style.configure("BG.TLabel",foreground="black",background="grey88", relief="groove")
        style.configure("BG.TEntry",foreground="black", background="grey88",fieldbackground="grey88")

        self.frame=frame

        self.title = tkinter.ttk.Label(self.frame,text=title, font =("Arial",9,"bold") )
        self.title.grid(sticky="W")

        #Set load/save buttons
        self.setfile=os.path.join(os.getcwd(),defaultfile)#defualt filename

        fileframe=tkinter.ttk.Frame(self.frame, relief="groove",borderwidth=6)
        fileframe.grid(columnspan=10,sticky="W")

        filelabel=tkinter.ttk.Label(fileframe,text="Load / Save", font =("Arial",8, "bold"))
        filelabel.grid(row=0,column=0,sticky="W",pady=0,padx=0)

        self.bfilename=tkinter.ttk.Button(fileframe,text="Load Query Set",command=self.set_filename)
        self.bfilename.grid(row=1,column=0,padx=2)

        self.bfilename=tkinter.ttk.Button(fileframe,text="Save Query Set",command= lambda: self.set_filename("save"))
        self.bfilename.grid(row=1,column=1,padx=2)


        self.bload=tkinter.ttk.Button(fileframe,text="Quick Load Query Set",command=self.loadfields)
        self.bload.grid(row=1,column=3,padx=2)

        self.bsave=tkinter.ttk.Button(fileframe,text="Quick Save Query Set",command=self.savefields)
        self.bsave.grid(row=1,column=4,padx=2)

        #"Safety catch" tickbox for quickload and quicksave
        self.safelabel=tkinter.ttk.Label(fileframe,text="Enable Quick Load/Save")
        self.safelabel.grid(row=0,column=3)
        self.safevar=tkinter.IntVar()
        self.safevar.set(0)
        self.safebox=tkinter.ttk.Checkbutton(fileframe, variable = self.safevar, command = self.safetycatch)
        self.safebox.grid(row=0,column=4,sticky="W")
        self.safetycatch()#

        #Show current filename
        self.flabel=tkinter.ttk.Label(fileframe,text="Selected File:")
        self.flabel.grid(row=1,column=5,padx=4,stick="E")
        self.lfilename=tkinter.ttk.Label(fileframe,text=os.path.basename(self.setfile),style="BG.TLabel")
        self.lfilename.grid(row=1,column=6,sticky="W")

        #Parameter fields
        row = self.frame.grid_size()[1]+1
        self.paramframe=tkinter.ttk.Frame(self.frame,relief="groove",borderwidth=6)
        self.paramframe.grid(row=row,columnspan=10,sticky="W")
        paramlabel=tkinter.ttk.Label(self.paramframe,text="Parameter Fields", font =("Arial",8, "bold"))
        ...
        param=tkinter.ttk.Combobox(self.paramframe,textvariable=self.paramvar, state="readonly", width=3)
        param.grid(row=0,column=1,sticky="S")
        param["values"]=("0","1","2","3","4","5","6")
        param.current(0)

        toprowlabel=tkinter.ttk.Label(self.paramframe,text="Title(s)")
        toprowlabel.grid(row=0,column=2)
        botrowlabel=tkinter.ttk.Label(self.paramframe,text="Value(s)")
        ...
        row=self.frame.grid_size()[1]+1
        self.qframe=tkinter.ttk.Frame(self.frame,relief="groove",borderwidth=6)
        self.qframe.grid(row=row,columnspan=10,sticky="W")

        #Title
        row=self.qframe.grid_size()[1]+1

        qtitle=tkinter.ttk.Label(self.qframe,text="Query Setup", font =("Arial",8, "bold"))
        qtitle.grid(row=row,column=0)

        #"Add" button
        self.bnew=tkinter.ttk.Button(self.qframe,text="Add Row",command=self.add_row)
        self.bnew.grid(row=row,column=1,padx=4)

        #Password masking tickbox
        pwhframe=tkinter.ttk.Frame(self.qframe)
        pwhframe.grid(row=row,column=3,columnspan=2,sticky='E')
        pmlabel = tkinter.ttk.Label(pwhframe,text="Hide Passwords")
        pmlabel.grid(row=0,column=0,sticky="W")
        self.pwhidevar=tkinter.IntVar()
        self.pwhidevar.set(1)
        pwhidebox=tkinter.ttk.Checkbutton(pwhframe, variable = self.pwhidevar, command = self.show_hide_password)
        pwhidebox.grid(row=0,column=1,sticky="W")

        #Reload SQL button
        self.bsqlreload = tkinter.ttk.Button(self.qframe,text="Reload SQL",command=self.reload_all_sql)
        self.bsqlreload.grid(row=row,column=5,padx=4)

        #Column headings for query rows
        self.headings = ["Include","Description","Database","Username","Password","SQL Query"]
        row=self.qframe.grid_size()[1]+1
        for i,h in enumerate(self.headings):
            l=tkinter.ttk.Label(self.qframe,text=h, font =("Arial",8,"bold"))
    ...
    def add_query(self):
        ...
        for h in (self.headings):
            ...
            if h=="Include":
                ...
                details["tickvar"].set(1)
                details["tick"]=tkinter.ttk.Checkbutton(self.qframe, variable = details["tickvar"])
                details["tick"].grid(row=row,column=col)

            #SQL
            elif h=="SQL Query":
                details["qbutton"] = tkinter.ttk.Button(self.qframe,text="Load", command = lambda id=details["id"]:self.sql_choose(id))
                details["qbutton"].grid(row=row, column=col, padx=(4,2),sticky='W')
                col=col+1
                details[h]=tkinter.ttk.Entry(self.qframe,width=68, style="BG.TEntry")
            ...
            #Password Fields
                ...
                if self.pwhidevar.get()==1:
                    ...
                    x=""
                details[h]=tkinter.ttk.Entry(self.qframe,width=14,show=x)
            ...
            #Default Entry Fields
                ...
                if h=="Description":
                    ...
                    width=16
                details[h]=tkinter.ttk.Entry(self.qframe,width=width)
                details[h].grid(row=row,column=col)

            col=col+1

        #Add delete button
        details["delbutton"] = tkinter.ttk.Button(self.qframe,text="Delete Row", command = lambda id=details["id"]:self.delete_row(id))
        details["delbutton"].grid(row=row,column=col,padx=(6,0))
    ...
    def delete_row(self,id):
        ...
        for e in self.qr[id]:
            if  self.qr[id][e].__class__ in [tkinter.ttk.Entry,tkinter.ttk.Button,tkinter.ttk.Label,tkinter.ttk.Checkbutton]:
    ...
    def readfields(self):
        ...
        for q in self.qr:
            ...
            for element in q:
                ...
                if q[element].__class__ in [tkinter.ttk.Entry,tkinter.IntVar]:
    ...
    def set_filename(self,mode=None):
        ...
        if mode=="save":
            ...
            filepath=tkinter.filedialog.asksaveasfilename(filetypes=[("DAT",".dat"),("All",".*")]
                ,title="Save - filename for Query Set",defaultextension=".dat")

        else:
            #Get "Load" filename
            filepath=tkinter.filedialog.askopenfilename(filetypes=[("DAT",".dat"),("All",".*")]
    ...
    def loadfields(self):
        ...
        for i,row in enumerate(self.qrd):
            for q in row:
                ...
                #Entry fieldss
                if element.__class__ == tkinter.ttk.Entry:
    ...
    def sql_choose(self,id,filename=None):
        ...
        #use fileselector r
            ...
            #Open dialogue shows previous file's location, if there is one
            lastpath = self.qr[id]["SQL Query"].get()
            lastpath = os.path.split(lastpath)[0]

            filepath=tkinter.filedialog.askopenfilename(filetypes=[("SQL",".sql"),("All",".*")]
            , initialdir=lastpath
    ...
    def read_sql(self,id):
        ...
        if filepath:
            ...
            try:
                ...
                tkinter.messagebox.showwarning("Load SQL","Failed to open file:"+filepath)
            else:
    ...
    def reload_all_sql(self):
        """Forces reload of all SQL files.
        Handy if original file updated or path on screen manually updated.
        Callback for button self.bsqlreload"""
        for q in self.qr:
            self.read_sql(q["id"])
    ...
    def paramupdate(self,*args):
        ...
        for i in range(0,n):
            ...
            e1=tkinter.ttk.Entry(self.paramframe, font =("Arial",8, "bold"))
            e1.grid(row=0,column=3+i)
            self.param_entry_names.append(e1)
            #Entries for parameter values
            e2=tkinter.ttk.Entry(self.paramframe)
...
class TestRunGui:
    ...
    def __init__(self,win_title="Query Thingy"):
        ...
        self.new_win = False
        self.results_win = None #separeate results window (when it exists)
        ...
        #Create canvas for Window scrollbars (separate from results scrollbars)
        ##self.wcanvas=Tkinter.Canvas(self.win)
        ##self.wcanvas.grid(sticky='nsew')

        #Set grid sizing behaviour (scroll bars don't work without this)
        self.win.grid_rowconfigure(2, weight=1)#Note row 2 is chosen here as we don't want incframe, runframe included
        self.win.grid_columnconfigure(0, weight=1)

        # Query Definition part of screen (at top)
        #Topframe that holds the query panel
        incframe=tkinter.ttk.Frame(self.win,borderwidth=6, relief="groove")
        ...
        runframe=tkinter.ttk.Frame(self.win,borderwidth=8, relief="groove")
        runframe.grid(sticky="W",pady=(4,4))#
        #Label
        title_label = tkinter.ttk.Label(runframe,text="Run Queries",font =("Arial",8, "bold") )
        title_label.grid(row=0,column=0,sticky="W")
        #'Start' button
        go_button = tkinter.ttk.Button(runframe,text="Start", command=self.go)
        go_button.grid(row=1,column=0,pady=0,sticky="w",padx=(0,40))#,columnspan=10)

        #Export Results button
        ex_button=tkinter.ttk.Button(runframe,text='Export Results',command=self.export_results)
        ex_button.grid(row=1,column=1,sticky="W",padx=0)
        ...
        rb1.grid(row=1,column=2,sticky="W",padx=0)
        rb2=tkinter.Radiobutton(runframe,text="TSV",variable=self.rbv,value="tsv")
        rb2.grid(row=1,column=3,sticky="W",padx=0)
        rb3=tkinter.Radiobutton(runframe,text="Text",variable=self.rbv,value="txt")
        rb3.grid(row=1,column=4,sticky="W",padx=(0,40))
        #Compare Button
        co_button = tkinter.ttk.Button(runframe,text="Simple Compare", command=self.docompare)
        co_button.grid(row=1,column=5,padx=0)
        #Align Button
        al_button = tkinter.ttk.Button(runframe,text="Align Columns", command=self.align)
        al_button.grid(row=1,column=6,padx=(4,40))

        #Display results in separate window tickbox
        #"Safety catch" tickbox for quickload and quicksave
        self.extwinlabel=tkinter.ttk.Label(runframe,text="Display results in external window")
        self.extwinlabel.grid(row=1,column=7)
        self.extwinvar=tkinter.IntVar()
        self.extwinvar.set(0)
        self.extwinbox=tkinter.ttk.Checkbutton(runframe, variable = self.extwinvar)
        self.extwinbox.grid(row=1,column=8,sticky="W")


        #Create Canvas scrollbars to work (results part of screen)
        self.rcanvas=tkinter.Canvas(self.win)
        self.rcanvas.grid(sticky='nsew')

        #Frame to show results within scrollable canvas
        self.resframe=tkinter.ttk.Frame(self.rcanvas,borderwidth=0, relief="groove")
        self.resframe.grid(sticky="W")

        #Create x and y scrollbars. Set their commands to canvas xview and yview
        rxscroll = tkinter.Scrollbar(self.win, orient=tkinter.HORIZONTAL, command=self.rcanvas.xview)
        rxscroll.grid(row=3, column=0, sticky='we')
        ryscroll = tkinter.Scrollbar(self.win, orient=tkinter.VERTICAL, command=self.rcanvas.yview)
        ryscroll.grid(row=2, column=1, sticky='ns')

        #Link the canvas x and y scrollcommands to the scrollbars
        self.rcanvas.configure(xscrollcommand=rxscroll.set,yscrollcommand=ryscroll.set)

        #Put the frame in the canvas scrollable zone
        self.rcanvas.create_window(0, 0, window=self.resframe, anchor='nw')
        ...
        self.rcanvas.configure(scrollregion=(0, 0, self.resframe.winfo_width(),self.resframe.winfo_height()))
    ...
    def go(self):
        ...
        #Set area for results to be displayed

        #Wipe old results frames
        for f in self.results_frames:
            f.destroy()

        #Handle whether results displayed in main or separate window
        #Display results in separate window
        if self.extwinvar.get() == 1:
            if self.results_win:
                pass
                #self.results_win.destroy()
            #Create new results window
            else:
                self.results_win = tkinter.Toplevel(self.win)
                self.results_win.title("Results")
                #Asign function automatically called when results window closed
                self.results_win.protocol("WM_DELETE_WINDOW",self.results_win_close)

                #Stuff mainly needed for scrollbars to work
                #Set grid sizing behaviour (scroll bars don't work without this)
                self.results_win.grid_rowconfigure(0, weight=1)
                self.results_win.grid_columnconfigure(0, weight=1)
                self.results_canvas = tkinter.Canvas(self.results_win)
                self.results_canvas.grid(row=0,column=0,sticky='NSWE')
                self.results_xscroll = tkinter.Scrollbar(self.results_win,orient=tkinter.HORIZONTAL,command=self.results_canvas.xview)
                self.results_yscroll = tkinter.Scrollbar(self.results_win,orient=tkinter.VERTICAL,command=self.results_canvas.yview)
                self.results_xscroll.grid(row=1, column=0, sticky='WE')
                self.results_yscroll.grid(row=0, column=1, sticky='NS')
                self.results_canvas.configure(xscrollcommand=self.results_xscroll.set,yscrollcommand=self.results_yscroll.set)
                self.ext_resframe = tkinter.ttk.Frame(self.results_canvas)
                self.results_canvas.create_window(0, 0, window=self.ext_resframe, anchor='nw')
                self.resframe.update_idletasks()
                self.results_canvas.configure(scrollregion=(0, 0, self.ext_resframe.winfo_width(), self.ext_resframe.winfo_height()))

            #Set destination to frame in external window
            self.destination_frame=self.ext_resframe
            #Shrink internal results frame as it's not currently needed
            self.resframe.config(height=1,width=1)
            self.resframe.update_idletasks()
            self.rcanvas.configure(scrollregion=(0, 0, self.resframe.winfo_width(),self.resframe.winfo_height()))

        #Set results to be displayed in main window
        else:
            #Set results destination to frame within main window
            self.destination_frame=self.resframe
            #Remove old results window if it exists
            self.results_win_close()

        #Create new results frames in designated destination location
        self.results_frames=[]
        for i,q in enumerate(self.queries):
            self.results_frames.append(tkinter.ttk.Frame(self.destination_frame,relief="groove",borderwidth=6))#create a frame for each query
        ...
        for i,q in enumerate(self.queries):
            ...
            if q.con!=currentcon:
                ...
                try:
                    ...
                    q.errors=[emessage]#assign the error to the query's errors attribute (so included when QueryData.writeframe(), or QueryData.makehtml() run)
                else:
                    cursor=cnxn.cursor()
                    currentcon=q.con
        ...
        #Update scrollable region (needed for scrollbars to respond to changes in window contents)
        self.destination_frame.update_idletasks()
        if self.results_win:
            canvas = self.results_canvas
            self.results_win.update()
        else:
            canvas = self.rcanvas
        canvas.configure(scrollregion=(0, 0, self.destination_frame.winfo_width(),self.destination_frame.winfo_height()))
    ...
    def export_results(self):
        ...
        savefile=tkinter.filedialog.asksaveasfilename(defaultextension=default,filetypes=types,title=heading)
    ...
    def align(self):
        """Callback for align button

        Sets column widths of results to match those
        from topmost query. Doesn't matter if
        other queries have fewer or more columns
        than the topmost query. Only columns common
        to both are affected.

        """
        #Only do something if we have defined queries
        if self.queries:
            #Find last column to be affected
            last=len(self.queries[0].colwidths)
            #Loop through queries other than the first
            for q in self.queries[1:]:
                #Loop through each column
                for wi in range(0,len(q.colwidths)):
                    #Only adjust if column also present in topmost query
                    if wi<last:
                        #Adjust column width to match corresponding column from topmost query
                        q.colwidths[wi]=self.queries[0].colwidths[wi]

            #Refresh the displayed results, so new widths displayed
            for qi, q in enumerate(self.queries):
                q.writeframe(self.results_frames[qi])

    def results_win_close(self):
        """Destroy external results window

        Only relevant if external results window being used
        """
        if self.results_win:
            self.results_win.destroy()
...
class QueryData:
    ...
    def clear(self):
        ...
        self.errors=[]#holds SQL error messages
    ...
    def execute(self,cursor,param=[]):
        ...
        try:
            ...
            print("SQL execution failed:",e[1])
            self.errors.append(str(e[1]))
        except pyodbc.Error as e:
            print("pyodbc error:",e[1])
            self.errors.append(str(e[1]))
        else:
        #Process results if SQL query completed successfully
    ...
    def writeframe(self,frame,extramessage="",max_rows=1000):
        ...
        Args:
            ...
            max_rows - maximum number of rows to display. Too many can crash tkinter
        """
        #Wipe any existing contents of the frame
        for w in list(frame.children.values()): w.destroy()

        #Title Text
        title=tkinter.ttk.Label(frame, text=self.title+" ("+self.runtime+")", font=("Helvetica",8,"bold"))#Title displayed using text lable
        ...
        for r,resultrow in enumerate(self.rows):#loop through all the data rows over chosen start-end range
            ...
            #Need to limit number of displayed rows. Get TCL error from grid when row number reaches 9999
            #Also GUI can crash on lower number
            if r< max_rows:#9996:
                ...
                for c,(col,cwidth) in enumerate(zip(resultrow,self.colwidths)):#loop through the row contents and corresponding collumn widths
                    ...
                    #Entry field to display element
                    ...
                    #Handling for "Row out of bounds" TclError although should be prevented by max_rows condition
                    try:
                        e.grid(row=r+2,column=c,sticky="W")#r+2 because row 0 is used for title text and row 1 for column headings
                    except tkinter.TclError as e:
                        print("Results grid geometry problem.")
                        print(e)
                        print("row:",r)
                    else:
                        e.insert(0, str(col))

            #Displayable row limit reached
            else:
                print("Too many rows to display. Tkinter limits reached.")
                # Display message in front-end
                e = tkinter.Entry(frame,font=("Helvetica",8,"bold"),width=128,bg='white',fg='red4')
                e.insert(0,'Results above limited to '+ str(max_rows)+' rows due to GUI limits. "Export Results" should still include all. Row count: '+str(len(self.rows)))
                e.grid(row=r+2,columnspan=1+len(self.headings),sticky="WE")
                break

        #Print message if no results returned
        if len(self.rows)==0:
            e=tkinter.Entry(frame,font=("Helvetica",8,"bold"),width=128,bg='white',fg='red4')#create widget to show message
            errors=" ".join(self.errors)
            e.insert(0,"No Results. "+errors+extramessage)
    ...
    def show(self):
        ...
        #Print headings
        for i,h in  enumerate(self.headings):
            print(h.ljust(self.colwidths[i]), end=' ')
        print("")
        #Print row data
        for r in self.rows:
            for i,c in enumerate(r):print(str(c).ljust(self.colwidths[i]), end=' ')
            print("")
        print("")
